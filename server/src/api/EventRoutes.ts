...
eventRoutes.get<EventsQuery, EventsResponse>("/:eventId?", authenticateToken, verifyIsClubAdmin, async (req, res) => {
    try {
...
eventRoutes.post<EventsQuery, Events>("/", authenticateToken, verifyIsClubAdmin, async (req, res) => {
    try {
...
eventRoutes.put<EventsQuery, Events>("/:eventId", authenticateToken, verifyIsClubAdmin, async (req, res) => {
    try {
        const eventId = Number(req.params.eventId);
        const { title, description, start_date, end_date } = req.body;
...
eventRoutes.delete<EventsQuery, void>("/:eventId", authenticateToken, verifyIsClubAdmin, async (req, res) => {
    try {
        const eventId = Number(req.params.eventId);
